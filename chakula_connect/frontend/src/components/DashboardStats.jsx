...
const DashboardStats = () => {
    ...
    const stats = useMemo(() => {
        return {
            ...
            totalVolume: orders.reduce((acc, curr) => acc + parseFloat(curr.quantity_ordered_kg), 0)
        };
    }, [orders]);

    return (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <StatsCard title={t.dashboardStats.activeDeliveries} value={stats.activeDeliveries} icon={Truck} color="text-blue-400" />
            <StatsCard title={t.dashboardStats.totalVolume} value={stats.totalVolume.toLocaleString()} icon={TrendingUp} color="text-green-400" />
